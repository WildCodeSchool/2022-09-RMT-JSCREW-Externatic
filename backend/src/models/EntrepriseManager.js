...
class EntrepriseManager extends AbstractManager {
  ...
  update(entreprise) {
    return this.connection.query(
      `update ${this.table} ,set logo = ?, set nom_entreprise = ?, set adresse = ?, set code_postal = ?, set ville = ?, set pays = ?, set email = ?, set telephone = ?, set description = ?, set numero_siret = ?, set dateInscription = ?, set domaine_id = ? where id = ?`,
      [
        entreprise.logo,
        entreprise.nom_entreprise,
        entreprise.adresse,
